...
from utils.inventory_helpers import add_item_to_inventory, activate_safe, activate_boost, activate_security, activate_roof, cleanup_phantom_inventory
    ...
    async for session in db.get_session():
        try:
            ...
            if _is_activatable(item.name):
                ...
                # Чистим возможные фантомные активируемые предметы
                await cleanup_phantom_inventory(session, user_id)
